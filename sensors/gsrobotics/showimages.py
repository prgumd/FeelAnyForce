...
def main(argv):
    ...
    dev = gsdevice.Camera("GelSight Mini R0B 2G1C-4J95")

    dev.connect()
    # dev.cam.set(cv2.CAP_PROP_BRIGHTNESS, 0)
    prop_ids = [i for i in range(1000)]
    properties = [(prop, dev.cam.get(prop)) for prop in prop_ids]
    print(properties)
    f0 = dev.get_raw_image()
    print(np.mean(cv2.cvtColor(f0, cv2.COLOR_BGR2HSV)[:,:,0]), np.mean(cv2.cvtColor(f0, cv2.COLOR_BGR2HSV)[1]),
          np.mean(cv2.cvtColor(f0, cv2.COLOR_BGR2HSV)[2]))
